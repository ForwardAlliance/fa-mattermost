...
- The composer's formatting toolbar (bold/italic/link/code/etc.) has no server-side setting either — Mattermost only exposes a personal, per-user toggle button for it — so `static/overrides.css` hides `[data-testid='formattingBarContainer']` and its toggle button (`#toggleFormattingBarButton`) to keep the composer simple by default.
...
- During the image build, `/mattermost/client/root.html` is patched to load `/static/overrides.css`, which hides Mattermost announcement bars and the initial boot splash screen (`#initialPageLoadingScreen`) globally, and `/static/overrides.js`, which rewrites the About popup's "join the community" link to point at this project's source repo instead of mattermost.com.
- On iOS Safari, tapping a button wrapped by Mattermost's `WithTooltip` (send, emoji picker, and most other icon buttons) opens the tooltip on the first tap instead of performing the action, because `WithTooltip` binds a real `mouseenter` listener straight onto the button and WebKit only sends `click` on a second, separate tap. `static/overrides.js` works around this by driving the click directly from `touchend` on the first tap and suppressing any native click that still follows, so the action always fires once per tap. This is a different WebKit quirk from the generic 300ms tap-delay/double-tap-zoom detection, which the same file's empty `touchstart` listener and `static/overrides.css`'s `touch-action: manipulation` handle separately.
